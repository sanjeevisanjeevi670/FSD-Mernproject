...
const AllProducts = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setVisibleProducts(Array.isArray(productsRes.data) ? productsRes.data : []);
    } catch (err) {
    ...
    try {
      const categoriesRes = await api.get('/api/products/fetch-categories');
  ...
  const handleSortFilterChange = (e) => {
    ...
    // avoid mutating visibleProducts in place; create new array before sorting
    if (value === 'low-price') {
      setVisibleProducts((prev) => [...prev].sort((a, b) => a.price - b.price));
    } else if (value === 'high-price') {
      setVisibleProducts((prev) => [...prev].sort((a, b) => b.price - a.price));
    } else if (value === 'discount') {
      setVisibleProducts((prev) => [...prev].sort((a, b) => b.discount - a.discount));
    } else {
      // popularity or default - fall back to products original order
      setVisibleProducts(products);
    }
  };

  useEffect(() => {
    // include `products` in dependencies because this effect uses it
  ...
  }, [categoryFilter, genderFilter, products]); // <-- added `products` here

  useEffect(() => {
    // derive visible products from products (example)
