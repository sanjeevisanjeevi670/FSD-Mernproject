...
// consolidate into one product object to avoid many unused setters
const [product, setProduct] = useState(null);


// ensure product state exists so setProduct is defined
...
const handleAddToCart = async() =>{
    await axios.post('http://localhost:6001/api/cart/add-to-cart', {userId, title: product.title, description: product.description, mainImg: product.mainImg, size, quantity: productQuantity, price: product.price, discount: product.discount}).then(
        (response)=>{
  ...
  useEffect(() => {
  async function fetchProductLocal() {
    try {
      const res = await fetch(`/api/products/${id}`); // adjust endpoint
      if (!res.ok) throw new Error('fetch failed');
      const data = await res.json();
      setProduct(data);
    } catch (err) {
      console.error('fetchProductLocal error', err);
    }
  }
  if (id) fetchProductLocal();
 }, [id]); // include id to satisfy ESLint
        ...
        <div className="IndividualProduct-body">
            ...
            <div id="carouselExampleIndicators" className="carousel slide" data-bs-ride="carousel">
                ...
                <div className="carousel-inner">
                    ...
                    <img src={product?.carouselImg1} className="d-block w-100" alt="..."  />
                    </div>
                    <div className="carousel-item">
                    <img src={product?.carouselImg2} className="d-block w-100" alt="..." />
                    </div>
                    <div className="carousel-item">
                    <img src={product?.carouselImg3} className="d-block w-100" alt="..." />
                    </div>
                </div>
            ...
            <div className="IndividualProduct-data">
                ...
                <span><h5><b>Price: </b> 	&#8377; {parseInt(product?.price - (product?.price * product?.discount)/100)}  </h5> <s>{product?.price}</s> <p>({product?.discount}% off)</p></span>
