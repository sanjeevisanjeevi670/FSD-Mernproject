...
const Cart = () => {
  ...
  // derive discount and delivery charges from cartItems (no unused setters)
  const totalDiscount = useMemo(() => {
    return (cartItems || []).reduce((sum, item) => {
      const qty = Number(item?.quantity ?? item?.qty ?? 1);
      const discountAmount = (Number(item?.price || 0) * Number(item?.discount || 0)) / 100;
      return sum + discountAmount * qty;
    }, 0);
  }, [cartItems]);

  const deliveryCharges = useMemo(() => {
    return (cartItems && cartItems.length > 0) ? 50 : 0; // adjust logic if needed
  }, [cartItems]);
  ...
  /* stable total calculator for useEffect deps */
  ...
  // single fetchCart implementation used by effects and removeItem
  ...
  /* recalc total whenever cartItems change */
  useEffect(() => {
    calculateTotalPrice();
  }, [calculateTotalPrice]);
